...
//         // Vérification champs 
//         // ISSET RETOURNE TRUE si la variable existe et qu'elle contient bien une autre valeur que NULL
...
if (!empty($_POST)) { // Si des données sont en POST
    ...
    if (!isset($_POST['email']) || strlen($_POST['email']) > 50 || !filter_var($_POST['email'], FILTER_VALIDATE_EMAIL) ) {
        // filter_var -> filtre de variable // dans ce filtre la fonction préféfinie FILTER_VALIDATE_EMAIL (c'est une constante elle est écrite en majuscule. Cette fonction vérifie que le format est bien un format email)
...
/   // Puis execution de la requete d'insertion avec les informations envoyés
    if (empty($contenu)) { // si la variable $contenu est vide = pas d'erreur
        $membre = executeRequete( " SELECT * FROM membre WHERE pseudo = :pseudo",  // on execute une requête SQL pour vérifier si le pseudo n'existe pas ds la BDD
        array(':pseudo' => $_POST['pseudo']));
        if ($membre->rowCount() > 0) { // si la requête retourne des lignes c'est que le pseudo existe déjà
            $contenu .= '<div class="alert alert-danger">le pseudo est indisponible veuillez en choisir un autre</div>';
        } else { // sinon on inscrit le membre en BDD
            $mdp = password_hash($_POST['mdp'], PASSWORD_DEFAULT);// cette fonction prédéfinie permet de crypter le mdp avec un l'algorithme dans la BDD.  
            // Lors de la connexion, il faudra comparer le mdp de la BDD avec celui du mdp de l'intérieur
            $succes = executeRequete(" INSERT INTO membre (pseudo, mdp, nom, prenom, email, civilite, adresse, code_postal, ville,  statut) VALUES (:pseudo, :mdp, :nom, :prenom, :email, :civilite, :adresse, :code_postal, :ville,  0)", 
                array(
                    ':pseudo' => $_POST['pseudo'],
